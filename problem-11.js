...
const getProduct = (row, col) => {
    ...
    var upLeft = row > 2 && col > 2 ?
    grid[row][col] * grid[row-1][col-1] * grid[row-2][col-2] * grid[row-3][col-3]
    :
    0;

    var downRight = row < grid.length - 3 && col < grid.length - 3 ?
    grid[row][col] * grid[row+1][col+1] * grid[row+2][col+2] * grid[row+3][col+3]
    :
    0;

    var upRight = row > 2 && col < grid.length - 3 ?
    grid[row][col] * grid[row-1][col+1] * grid[row-2][col+2] * grid[row-3][col+3]
    :
    0;

    var downLeft = row < grid.length - 3 && col > 2 ? 
    grid[row][col] * grid[row+1][col-1] * grid[row+2][col-2] * grid[row+3][col-3]
    :
    0;

    return Math.max(up, down, left, right, upLeft, upRight, downLeft, downRight);
}

var max = 0;
for(var i = 0; i < grid.length; i++) {
    for(var j = 0; j < grid[i].length; j++) {
        max = Math.max(max, getProduct(i,j));
    }
}
